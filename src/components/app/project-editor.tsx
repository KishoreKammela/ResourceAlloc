...
export default function ProjectEditor({ project, allEmployees }: { project: Project, allEmployees: Employee[] }) {
  ...
  const onSubmit: SubmitHandler<ProjectFormValues> = async (data) => {
    setIsSaving(true);
    try {
      const result = await handleUpdateProject(project.id, { ...data, requiredSkills, team });
      if (result.error) {
  ...
  const availableEmployees = allEmployees.filter(emp => !team.some(member => member.id === emp.id));

  return (
    <Card className="max-w-4xl mx-auto shadow-lg">
      <CardHeader>
        <CardTitle className="font-headline text-2xl">Edit Project</CardTitle>
        <CardDescription>Update the details for "{project.name}".</CardDescription>
      </CardHeader>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <CardContent className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Provide a brief overview of the project." {...field} rows={4}/>
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <div className="space-y-4">
                <h3 className="text-lg font-headline font-semibold">Manage Required Skills</h3>
                <div>
                  ...
                  <div className="p-4 border rounded-md min-h-[80px] flex flex-wrap gap-2">
                    ...
                    {requiredSkills.length === 0 && <p className="text-sm text-muted-foreground">No skills assigned. Add some below.</p>}
                  </div>
                </div>
                 <div>
            ...
            <Separator />
            <div className="space-y-4">
                <h3 className="text-lg font-headline font-semibold">Manage Team</h3>
                 <div>
                  <FormLabel>Current Team Members</FormLabel>
                  <div className="p-4 border rounded-md min-h-[80px] space-y-3">
                    {team.map(member => (
                      <div key={member.id} className="flex items-center justify-between">
                        ...
                        <Button type="button" variant="ghost" size="icon" onClick={() => removeTeamMember(member.id)}>
                ...
                <div>
                    ...
                    <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
                        <PopoverTrigger asChild>
                            <Button type="button" variant="outline" role="combobox" aria-expanded={popoverOpen} className="w-full justify-start">
                                ...
                                <CommandList>
                                    ...
                                    <CommandGroup>
                                        {availableEmployees.map(employee => (
                                            <CommandItem key={employee.id} onSelect={() => addTeamMember(employee)} className="flex items-center justify-between">
                                                <span>{employee.name} - <span className="text-muted-foreground">{employee.title}</span></span>
