...
const SkillGapAnalysisOutputSchema = z.object({
    missingSkills: z.array(z.string()).describe("A list of skills that are required by projects but are not available in the employee pool."),
    analysisSummary: z.string().describe("A brief, insightful summary of the overall skill landscape, including strengths and key weaknesses."),
    trainingSuggestions: z.array(z.object({
        skill: z.string().describe("The skill that training is being suggested for."),
        suggestion: z.string().describe("A brief, actionable training suggestion to help bridge this specific skill gap."),
    })).describe("A list of targeted training suggestions to address the identified skill gaps."),
});
export type SkillGapAnalysisOutput = z.infer<typeof SkillGapAnalysisOutputSchema>;

export async function performSkillGapAnalysis(input: SkillGapAnalysisInput): Promise<SkillGapAnalysisOutput> {
